...
async def play(bot, ctx):
    ...
    while alive:
        ...
        if (
            world[userRow - 1][userCol] == "w"
            or world[userRow + 1][userCol] == "w"
            or world[userRow][userCol - 1] == "w"
            or world[userRow][userCol + 1] == "w"
        ):
            # p_msg
        ...
        # Tells user if he is near a pit
            ...
            world[userRow - 1][userCol] == "p"
            or world[userRow + 1][userCol] == "p"
            or world[userRow][userCol - 1] == "p"
            or world[userRow][userCol + 1] == "p"
        ):
            # p_msg
        ...
        # Tell the user if he is near a bat
            ...
            world[userRow - 1][userCol] == "b"
            or world[userRow + 1][userCol] == "b"
            or world[userRow][userCol - 1] == "b"
            or world[userRow][userCol + 1] == "b"
        ):
            # p_msg
        ...
        def check(reaction, user):
            ...
            return user != bot.user and (
                str(reaction.emoji) == "?" or "?" or "?" or "?" or "?" or "?"
            )
        ...
        if action == "?":
            ...
            def checkf(reaction, user):
                return (
                    user != bot.user
                    and user == ctx.author
                    and (str(reaction.emoji) == "?" or "?" or "?" or "?")
                )

            try:
                flight, user = await bot.wait_for(
                    "reaction_add", timeout=10.0, check=checkf
                )
            except asyncio.TimeoutError:
                await ctx.send(f"Time's Up! :stopwatch:\nGame over :coffin:")
                await endBoard(userRow, userCol, sys_msg)
            ...
            if arrows == 0:
                await ctx.send(
                    "You're out of arrows :bow_and_arrow:\nGame over :coffin:"
                )
                await endBoard(userRow, userCol, sys_msg)
                return

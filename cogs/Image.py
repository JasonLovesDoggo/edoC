...
from utils.cooldowns import api_cooldown, pfp_cooldown
...
class Image(Cog, description="Image Related commands are here"):
    ...
    async def get_r_animal(self, ctx, endpoint: str):
        ...
        async with ctx.session.get(
            f"https://some-random-api.ml/animal/{endpoint}"
        ) as r:
            if r.status != 429:
    ...
    async def cat(self, ctx):
        ...
        async with ctx.session.get(f"{base_url}/cat?json=true") as resp:
            ...
            await ctx.send(
                embed=discord.Embed()
                .set_image(url=base_url + js["url"])
                .set_footer(text=f"Tags: {feet}" if len(feet) > 1 else "")
            )
    ...
    async def lizard(self, ctx):
        ...
        async with ctx.session.get("https://nekos.life/api/v2/img/lizard") as api:
            data = await api.json()
        emb = discord.Embed(title="Lizard", color=invis)
        emb.set_image(url=data["url"])
        await ctx.send(embed=emb)

    @command(
        aliases=["MyDoggo", "Bella", "Belz", "WhosAgudGurl"],
        brief="Posts a random pic of my doggo Bella :)",
    )
    ...
    async def supreme(self, ctx, *, text: str):
        embed = Embed(
            title=f"Rendered by {ctx.author.display_name} VIA {ctx.guild.me.display_name}",
            color=invis,
        ).set_image(url="attachment://supreme.png")
        image = File(
            await (await self.bot.alex_api.supreme(text=text)).read(), "supreme.png"
        )
    ...
    @command(
        brief="Gives you a random red panda.",
        aliases=["rpanda", "redpand", "red-panda", "redpanda"],
    )
    @dynamic_cooldown(api_cooldown, BucketType.user)
    ...
    @pfp.command(
        breif="pixelates a pfp",
        name="pixelate",
        aliases=["pixel", "pixels", "pixelify", "makepixely"],
    )
    @dynamic_cooldown(pfp_cooldown, BucketType.user)
    ...
    @pfp.command(
        name="triggered",
        aliases=["trig", "triggers"],
        brief="sends a img of the dudes pfp triggered",
    )
    @dynamic_cooldown(pfp_cooldown, BucketType.user)

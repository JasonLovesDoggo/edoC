...
class Fun(commands.Cog, description="Fun and entertaining commands can be found below"):
        ...
        try:
            while True:
                msg = await self.bot.wait_for(
                    "message", timeout=15, check=lambda m: m.id != message.id
                )
                if str(answer) in msg.content:
                    return await ctx.success(
                        f"{answer} was correct! ({question.answer})"
                    )
    ...
    async def rolldice(self, ctx, guess):
        ...
        await ctx.reply(
            embed=discord.Embed(
                color=green if guess == answer else red,
    ...
    async def mc(self, ctx, *, txt: str):
        ...
        author = (
            ...
            if len(ctx.author.display_name) < 22
    ...
    async def coinflip(self, ctx, *, toss="Heads"):
        ...
        if len(toss) > 100:
            return await ErrorEmbed(
                ctx=ctx, err="Please keep the length of your toss down"
            )
        ...
        embed.add_field(
            ...
            value="Someone is gonna go cry to mommy.",
            inline=False,
        )
    ...
    async def inspire(self, ctx):
        ...
        await ctx.send(
            embed=discord.Embed(description=data2[0]["q"], color=invis).set_author(
                name=data2[0]["a"]
            )
        )
    ...
    async def iseven(self, ctx, num: int):
        ...
        async with ctx.session.get(
            f"https://api.isevenapi.xyz/api/iseven/{num}/"
        ) as api:
            data = await api.json()
        if data["iseven"]:
            color = green
    ...
    async def choosebestof(
        ...
        await ctx.send(
            file=discord.File(data, filename=f"{CustomTimetext('prolog', 'output')}")
        )
    ...
    async def f(self, ctx, *, text: commands.clean_content = None):
        ...
        await ctx.send(
            f"**{ctx.author.name}** has paid their respect {reason}{choice(hearts)}"
        )
    ...
    async def nuke(self, ctx):
        ...
        if (
            ...
            or ctx.channel.permissions_for(ctx.author).manage_channel
        ):
    ...
    async def rate(self, ctx, *, thing: commands.clean_content):
        ...
        re = abs(hash(thing[-1]))
        await ctx.send(
            f"I'd rate `{thing}` a **{int(str(rateamount)[:2])}.{str(re)[:2]} / 100**"
        )
    ...
    async def countdown(self, ctx, times: int = 3, *, tosay: str = "Go Go Go"):
        ...
        if times > 10:
            return await ctx.reply(
                embed=discord.Embed(
                    color=error, description="The number must be between 0 and 10"
                )
            )
    ...
    @commands.command()
    async def ship(
        self, ctx, person1: commands.clean_content, person2: commands.clean_content
    ):
        """Rates what you desire"""
        ...
        await ctx.send(
            f"`{person1}` and `{person2}` are **{round(ship_amount, 4)} compatible **"
        )
    ...
    @commands.command()
    async def beer(
        self, ctx, user: MemberConverter = None, *, reason: commands.clean_content = ""
    ):
        """Give someone a beer! 🍻"""
        ...
        try:
            ...
            return await ctx.unknown(
                "uhm unknown error with the beer command", ctx.command
            )
    ...
    async def urban(self, ctx, *, term: str):
        ...
        """
        res = await ctx.session.get(
            "http://api.urbandictionary.com/v0/define", params={"term": term}
        )
        if not res.ok:
            await ctx.send(
                f"An error occured at the API. Try again with a different word or wait a bit."
            )
            return
        ...
        if not data["list"]:
            await ctx.send(
                f"No results were found for term `{term}`. Try again with a different word."
            )
            return

        menu = MenuPages(
            source=UrbanSource(data["list"]), timeout=30, delete_message_after=True
        )
    ...
    async def noticeme(self, ctx):
        ...
        await ctx.send(
            embed=discord.Embed(color=0xFF6D7A).set_image(
                url="https://i.alexflipnote.dev/500ce4.gif"
            )
        )

...
class Menu(ListPageSource):
    ...
    async def write_page(self, fields=[]):
        ...
        embed.set_thumbnail(url=self.ctx.guild.me.display_avatar.url)
...
class Info(Cog, description="Informational and useful commands"):
    ...
    async def mcinfo(self, ctx, user: str):
        ...
        em = Embed(color=random_color()).set_image(
            url=f"https://minotar.net/armor/body/{prof.name}/550.png"
        )
        em.description = (
            f"**UUID :** {uuid}\n"
        ...
        em.set_author(
            name=f"Info About {prof.name}",
            url=f"https://namemc.com/profile/{prof.name}",
    ...
    @command(
        aliases=["spelling", "fixspelling", "autocorrect"],
        brief="Corrects the spelling of the input",
    )
    ...
    async def spotifyinfo(self, ctx, user: MemberConverter = None):
        ...
        if not spotify:
            return await ctx.error(f"{user} is not listening to Spotify!")
    ...
    async def create_embed(self, description, field: List[Tuple] = None):
        embed_ = discord.Embed(description=description, colour=orange)
        ...
        if message is None:
            ...
            if ref and isinstance(ref.resolved, discord.Message):
                ...
                return await ctx.reply(
                    embed=discord.Embed(
                        description="Missing a message to translate", color=error
                    )
                )
    ...
    async def _report(self, ctx):
        ...
        if answers:
            em = Embed(
                title=answers[0], description=answers[1], timestamp=datetime.utcnow()
            )
    ...
    async def _suggest_feature(self, ctx, *, suggestion: str):
        ...
        await msg.add_reaction("👎")
        await ctx.send(
            f"Submitted your suggestion. Join the support server to see the status of your suggestion."
        )
    ...
    async def RandomColor(self, ctx):
        ...
        embed.set_thumbnail(
            url=f"https://api.popcatdev.repl.co/color/image/{hex_number}?width=100&height=100"
        )
    ...
    async def Color(self, ctx, colorname: ColourConverter):
        ...
        embed.add_field(name="Rgb value", value=data["rgb"][3:], inline=False)
        embed.set_thumbnail(
            url=f"https://api.popcatdev.repl.co/color/image/{strcolor}?width=100&height=100"
        )
        await ctx.send(embed=embed)
    ...
    async def todomenu(self, ctx, command):
        embed = discord.Embed(
            title=f"TODO", description="test", colour=ctx.author.colour
        )
    ...
    async def todo(self, ctx):
        ...
        items = []
        results = sorted(
            (await self.get_all_todo(ctx.author.id)), key=lambda x: x["time"]
        )
        for each in results:
    ...
    async def add(self, ctx, *, todo):
        ...
        await self.bot.db.execute(
            "INSERT INTO todo (todo, id, time, message_url, user_id) VALUES ($1, $2, $3, $4, $5)",
            todo,
            id,
            time(),
        ...
        await ctx.send(
            f"{ctx.tick()} Inserted `{todo}` into your todo list! (ID: `{id}`)"
        )
    ...
    async def resolve(self, ctx, *id: int):
        ...
        for i in id:
            ...
            await self.bot.db.execute(
                "DELETE FROM todo WHERE user_id = $1 AND id = $2", ctx.author.id, i
            )
        await ctx.send(
        ...
        num = len(
            (
                await self.bot.db.fetch(
                    "SELECT * FROM todo WHERE user_id = $1", ctx.author.id
                )
            )
        )
    ...
    async def contributors(self, ctx):
        emb = discord.Embed(
            title=f"Contributors Of edoC", color=random_color(), description=""
        )
        emb.set_footer(
            text="Created by Jake CEO of annoyance#1904",
    ...
    async def donate(self, ctx):
        class DonateView(discord.ui.View):
            def __init__(self):
                ...
                self.add_item(
                    discord.ui.Button(label="Ko-Fi", url="https://ko-fi.com/edocthebot")
                )
                self.add_item(
                    discord.ui.Button(
                        label="Buy me a coffee", url="https://www.buymeacoffee.com/edoC"
                    )
                )
                self.add_item(
                    discord.ui.Button(
                        label="Donate Bot",
                        url="https://donatebot.io/checkout/819282410213605406",
                    )
                )
    ...
    async def _in(self, ctx):
        ...
        class InfoView(discord.ui.View):
            def __init__(self):
                ...
                self.add_item(
                    discord.ui.Button(
                        label="Website", url="https://jakewaschosen.github.io/edocSite/"
                    )
                )
                self.add_item(
                    discord.ui.Button(
                ...
                self.add_item(
                    discord.ui.Button(label="Dev's links", url="https://bio.link/edoC")
                )
                self.add_item(
                    discord.ui.Button(
                        label="Support me",
        ...
        infos[f"{status(str(ctx.guild.me.status))} Uptime"] = precisedelta(
            discord.utils.utcnow() - self.bot.start_time, format="%.0f"
        )
        infos[
            "System"
...
Comments: {comments}"""
        ...
        async with ctx.channel.typing():
            ...
            infemb.set_thumbnail(url=ctx.guild.me.display_avatar.url)
            ...
            infemb.add_field(
                name="<:edoC:874868276256202782> edoC Version",
    ...
    async def sendasformatedfile(self, ctx, filetype: str, *, text: str):
        ...
        data = BytesIO(text.encode("utf-8"))
        await ctx.reply(
            file=discord.File(data, filename=f"{CustomTimetext(filetype, 'Text')}")
        )
        ...
        class SourceView(discord.ui.View):
            ...
            async def delete(
                ...
                if interaction.user != self.ctx.author:
                    return await interaction.response.send_message(
                        "Oops. This is not your interaction.", ephemeral=True
                    )
            ...
            async def send_file(
                ...
                await interaction.channel.send(
                    file=discord.File(
                        BytesIO(dedent("".join(lines)).encode("ascii")), filename
                    )
                )
    ...
    @command(
        help="Shows the bot's latency in miliseconds Useful if you want to know if the bot is lagging or not",
    ...
    async def botstats(self, ctx):
        ...
        info["Prefix in this server"] = await self.bot.get_prefix(
            ctx.message
        )  # self.db.fetch('SELECT prefix FROM prefixs WHERE id = ?', ctx.guild.id)
        ...
        await ctx.send(
            content=f"About **{ctx.bot.user}** | **{version_info['version']}**",
            embed=em,
        )

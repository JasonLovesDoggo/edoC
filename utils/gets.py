...
def getAgo(time):
    ...
    if 120 > sec:
        ...
        return (
            "{} hour ago".format(sec // 60 // 60)
            if 7200 > sec
            else "{} hours ago".format(sec // 60 // 60)
        )
    else:
        return (
            "{} day ago".format(sec // 60 // 60 // 24)
            if 7200 > sec
            else "{} days ago".format(sec // 60 // 60 // 24)
        )
...
def getUser(ctx, msg):
    if msg == "":
        ...
        return utils.find(
            lambda m: msg.lower() in m.name.lower(), ctx.message.guild.members
        )
    else:
...
def getChannel(ctx, msg):
    if msg == "":
        ...
        return utils.find(
            lambda c: msg.lower() in c.name.lower(), ctx.guild.text_channels
        )
    else:
        return utils.find(
            lambda c: msg.lower() in c.name.lower(), ctx.bot.get_all_channels()
        )
...
def getRole(ctx, msg):
    ...
    if 1 == len(ctx.message.role_mentions):
        ...
        return utils.find(
            lambda r: msg.strip().lower() in r.name.lower(), ctx.guild.roles
        )
...
def getColor(incolor):
    if len(incolor.split(",")) == 3:
        try:
            ...
            if (
                float(incolor[0]) > 1.0
                or float(incolor[1]) > 1.0
                or float(incolor[2]) > 1.0
            ):

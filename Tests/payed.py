...
class Investing:
    ...
    async def _get_session(self):
        c = await self.session.get(
            url=f"https://www.myfxbook.com/api/login.json?email={self._email}&password={self._password}"
        )
        data = await c.json()
    ...
    async def get_outlook(self):
        ...
        async with self.session.get(
            f"https://www.myfxbook.com/api/get-community-outlook-by-country.json?session={self._session_id_}&symbol=eurusd"
        ) as c:
            data = await c.json()
        l = []
